    ...
    if ($reservas->num_rows == 0) {
        ...
        while ($reserva = $reservas->fetch_assoc()) {
            ...
            if ($reserva['aprovado'] == 1) {
                echo "<tr><td>{$sala['nome']}</td><td>{$reserva['data']}</td><td>{$tempo['horashumanos']}</td><td><span class='badge bg-success' data-bs-toggle='tooltip' data-placement='top' title='A sua reserva foi aprovada! Um email foi lhe enviado com mais informações.'>Aprovado</span></td><td><a class='btn btn-secondary' href='/reservar/manage.php?tempo={$reserva['tempo']}&sala={$reserva['sala']}&data={$reserva['data']}'>Detalhes</a></td></tr>";
            } else if ($reserva['aprovado'] == -1) {
                echo "<tr><td>{$sala['nome']}</td><td>{$reserva['data']}</td><td>{$tempo['horashumanos']}</td><td><span class='badge bg-danger' data-bs-toggle='tooltip' data-placement='top' title='Foi lhe enviado um email com mais informações sobre a rejeição.'>Rejeitado</span></td><td><a class='btn btn-secondary' href='/reservar/manage.php?tempo={$reserva['tempo']}&sala={$reserva['sala']}&data={$reserva['data']}'>Detalhes</a></td></tr>";
            } else {
                echo "<tr><td>{$sala['nome']}</td><td>{$reserva['data']}</td><td>{$tempo['horashumanos']}</td><td><span class='badge bg-warning' data-bs-toggle='tooltip' data-placement='top' title='A sua reserva foi enviada e está a ser revista. Irá receber um email com mais informações em breve'>Pendente</span></td><td><a class='btn btn-secondary' href='/reservar/manage.php?tempo={$reserva['tempo']}&sala={$reserva['sala']}&data={$reserva['data']}'>Detalhes</a></td></tr>";
            }
        }
        echo "</table></div>";
    }
